...
filename = "XIth_BIO-BOTANY_TM_Combined-Page_326-336_U04.xlsx"
...
def create_slide(quizRecord, prs, isQuestion=True):
    ...
    shape = slide.shapes.add_shape(
        MSO_SHAPE.RECTANGLE, 0, Inches(0.3), Inches(10), Inches(0.8)
    )
    ...
    run.text =  quizRecord.topic_name
    ...
    WasPreviouslyAsked = getOptionAsString(currentQuiz.exams_already_asked)
    if  WasPreviouslyAsked != "":
        prg = tb.add_paragraph()
        prg.text = getOptionAsString(
            currentQuiz.exams_already_asked) + " - " + getOptionAsString(currentQuiz.year)
        prg = tb.add_paragraph()
        prg.text = ""
