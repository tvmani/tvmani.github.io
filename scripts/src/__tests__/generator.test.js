...
test("Stateful shuffled number generator", () => {
  fc.assert(
    fc.property(fc.tuple(fc.integer(1, 9), fc.integer(11, 19)), (inputs) => {
      //min, max, excludes, steps
      const generator = Generator.getStatefulShuffledGenerator(inputs[0], inputs[1], [7, 9], 2);
      let a = generator.getNext();
      let b = generator.getNext();
      expect(a).not.toBe(-1);
      expect(a).not.toBe(b);      
    })
  , {verbose: true});
