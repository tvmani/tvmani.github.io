...
function GetResults(){
    ...
    let selectedYear = yearSelect.value.toString();

    // Get last 2 digits of the year
    let year2Digits = selectedYear.substr(-2);
    ...
    resultStr = resultStr + `4. Anchor number for  year : ${selectedYear} is  = ${anchorNum}\n`;

    let calc4 = calc1 + calc2 + calc3 + anchorNum;
 
    resultStr = resultStr + "5. Add all results plus the anchor day value : " + calc1 + " + " + calc2 + " + " + calc3 + " + " + anchorNum + " = " + calc4 + "\n";
 
    // Take modulus of 7 from previous result to get a value between
    // 0 and 6 representing the days of a week (The Doomsday)
    let calc5 = calc4 % 7;
 
    resultStr = resultStr + "6. Take the modulus of 7 from the previous result to get Doomsday : " + calc4 + " % 7 = " + calc5 + " (" + dayArr[calc5] + ")" + "\n"; 
    ...
    resultStr = resultStr + `7. Is it a leap year? ` + leapYear.toString() + `\n (${year} % 4 === ${year % 4} AND ${year} mod 100 !== ${year % 100}  AND  ${year} mod 400 == ${year % 400}) \n`;
    ...
    resultStr = resultStr + `8. Add or Subtract previous result from the Doomsday number to find this dates day : ${calc5} \n`;
    ...
    if(leapYear){
        ...
        resultStr = resultStr + `9. Doomsday for  leapyear for the ${monthSelected} of the year ${selectedYear} : ${doomsdayDay}\n`;
    } else {
        doomsdayDay = doomsdayArrayNotLeapYear[monthIndex];
        resultStr = resultStr + `9. Doomsday for  non-leapyear for the ${monthSelected} of the year ${selectedYear} : ${doomsdayDay}\n`;
    }
    ...
    if(day < doomsdayDay){
        ...
        resultStr = resultStr + calc5 + " - (" + doomsdayDay + " - " + day + ")";
        ...
        resultStr = resultStr + `(${calc5} + ( ${day}  - ${doomsdayDay})) % 7`;
    } else {
        resultStr = resultStr + `\n Selected day matches doomsday ${doomsdayDay}`;
        dayIndex = calc5;
    }
